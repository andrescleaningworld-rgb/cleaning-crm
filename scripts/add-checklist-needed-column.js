#!/usr/bin/env node
/**
 * One-time setup script: adds a "Checklist Needed" header cell to the
 * existing "Accounts" tab in GOOGLE_MAIN_SHEET_ID, in the first empty column
 * after whatever headers are already there. Never touches any existing
 * header or data row. Safe to re-run — if "Checklist Needed" already exists
 * anywhere in row 1, it just reports that column and exits.
 *
 * Prints the resulting column letter/index — wire that into MAIN_COL /
 * fetchMainRows in lib/googleSheets.ts after running this.
 *
 * Usage:
 *   node scripts/add-checklist-needed-column.js
 *
...
const TAB_NAME = "Accounts";
const NEW_HEADER = "Checklist Needed";
...
async function main() {
  ...
  const accountsSheet = (meta.data.sheets ?? []).find((s) => s.properties?.title === TAB_NAME);
  const gridColumnCount = accountsSheet?.properties?.gridProperties?.columnCount ?? 0;
      ...
      requestBody: {
        requests: [{
          updateSheetProperties: {
            properties: { sheetId: accountsSheet.properties.sheetId, gridProperties: { columnCount: nextColOneBased } },
  ...
  console.log(`0-based column index (for MAIN_COL): ${nextColOneBased - 1}`);
  console.log("Done.");
}
