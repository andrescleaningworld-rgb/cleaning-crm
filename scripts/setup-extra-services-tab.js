#!/usr/bin/env node
/**
 * One-time setup script: adds the ExtraServices tab (with header row) to the
 * spreadsheet used by lib/googleSheets.ts's customer-portal-facing tabs
 * (GOOGLE_SHEET_ID) — the same spreadsheet that already hosts customer-portal,
 * portal-complaints, portal-service-requests, portal-date-changes, and
 * portal-billing-requests. Safe to re-run — skips creation if the tab already
 * exists and never touches any other tab.
 *
 * Usage:
 *   node scripts/setup-extra-services-tab.js
 *
...
const TAB_NAME = "ExtraServices";
// Column order must match EXTRA_SERVICE_COL in lib/googleSheets.ts (A-F).
const HEADERS = ["Id", "Name", "Description", "ImageUrl", "Active", "SortOrder"];
