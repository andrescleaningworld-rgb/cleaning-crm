#!/usr/bin/env node
/**
 * One-time setup script: adds the subcontractor-visits tab (with header row)
 * to the spreadsheet used by lib/googleSheets.ts's VISITS_TAB (GOOGLE_SHEET_ID).
 * Safe to re-run — skips creation if the tab already exists and never touches
 * any other tab.
 *
 * This tab was referenced in code (getAllSubcontractorVisits,
 * getSubcontractorVisits, logSubcontractorVisit, etc.) but was never actually
 * created in the spreadsheet, which is why every read/write to it has failed
 * with "Unable to parse range" since the feature was introduced.
 *
 * Usage:
 *   node scripts/setup-subcontractor-visits-tab.js
 *
...
const TAB_NAME = "subcontractor-visits";
// Column order must match VISIT_COL in lib/googleSheets.ts (A-G).
const HEADERS = [
  "VisitId", "AccountName", "SubEmail", "SubName",
  "VisitDate", "ArrivalTime", "Notes",
];
